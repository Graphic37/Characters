...
## Size
    ...
    src   1611 KB   (573 KB gzipped)
    dist  1117 KB   (397 KB gzipped)   -31%

**Most of the file is comments — about 21%.** They are not filler: they record
why each decision was made, and several are the only surviving account of a bug
that took multiple sessions to find. They stay in `src/`.

**For GitHub, size is mostly a non-issue.** Git stores deltas and GitHub serves
gzipped, so the practical cost of the comments is the ~176 KB difference in the
compressed column, not the raw megabyte. A 1.6 MB text file is unremarkable in a
repository.
...
and produced a file that looked fine and would have booted to a black screen.
A single-file game with no build step has nothing downstream to catch that, so
the script now refuses to write at all if a parser is unavailable.
...
built game plays identically — terser preserves semantics, but nothing here
demonstrates it for this file. Load `dist/` once before relying on it.
